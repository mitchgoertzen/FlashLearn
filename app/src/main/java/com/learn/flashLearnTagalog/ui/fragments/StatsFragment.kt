...
class StatsFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        lessonsButton.setOnClickListener{
            ...
            transaction?.replace(R.id.main_nav_container, fragment)?.addToBackStack("lesson stats")?.commit()
        ...
        wordsButton.setOnClickListener{
            ...
            val transaction = activity?.supportFragmentManager?.beginTransaction()
            transaction?.replace(R.id.main_nav_container, fragment)?.addToBackStack("word stats")?.commit()
            (activity as LearningActivity?)?.transitionFragment()
        }
